...
const nlMessagesObservable: WritableObservable<Translations> = observable(nlMessages);

if (viteHotContext?.hot) {
  // Hot reloading, see translations-hmr.ts
  viteHotContext.hot.accept(translationHotReload(nlMessagesObservable));
}

export default nlMessagesObservable.readOnly();
